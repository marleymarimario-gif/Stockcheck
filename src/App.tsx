...
const appVersion = "2026.08.14.11";
...
function CorrectStockInDialog({ entry, items, workspace, close, saved }: { entry: Activity; items: InventoryItem[]; workspace: Workspace; close: () => void; saved: () => Promise<void> }) {
  ...
  const [form, setForm] = useState({ productId: entry.product_id, productName: initialItem?.name ?? entry.product_name, category: initialItem?.category ?? categories[0] ?? "", quantity: String(entry.entered_quantity), unitMode: (entry.entered_unit === "箱／包" ? "package" : "base") as UnitMode });
  const [newCategory, setNewCategory] = useState(false);
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  ...
  const save = async () => {
    if (!item || !form.productName.trim() || !form.category.trim() || !Number.isInteger(enteredQuantity) || enteredQuantity <= 0) return setError("請填妥產品資料及正確數量");
    setBusy(true);
    const { error: updateError } = await supabase.rpc("correct_stock_in", { target_workspace: workspace.id, target_stock_in: entry.id, target_product: item.id, target_quantity: enteredQuantity, target_unit_mode: form.unitMode, target_product_name: form.productName.trim(), target_category: form.category.trim() });
    setBusy(false); if (updateError) return setError("未能更正入貨；請確認你有操作權限"); await saved();
  };
  return <div className="modal-backdrop"><section className="modal correction-modal"><div className="modal-head"><div><p className="eyebrow">保留原記錄</p><h2>更正入貨</h2></div><button onClick={close}>取消</button></div><div className="original-entry"><span>原本記錄</span><strong>{entry.original_product_name ?? entry.product_name} · +{entry.original_quantity ?? entry.quantity}</strong><small>{entry.actor} · {new Date(entry.happened_at).toLocaleString("zh-HK")}</small></div><div className="modal-form"><label>正確產品<select value={form.productId} onChange={(event) => { const product = items.find((candidate) => candidate.id === event.target.value); if (product) setForm({ ...form, productId: product.id, productName: product.name, category: product.category }); setNewCategory(false); }}>{items.map((product) => <option key={product.id} value={product.id}>{product.category}｜{product.brand}｜{product.flavor}</option>)}</select></label><label>產品名稱<input value={form.productName} onChange={(event) => setForm({ ...form, productName: event.target.value })} /></label><div className="category-picker"><div className="category-picker-head"><span>種類</span><button onClick={() => { setNewCategory((current) => !current); setForm({ ...form, category: newCategory ? (item?.category ?? categories[0] ?? "") : "" }); }}>{newCategory ? "顯示現有分類" : "＋ 新增分類"}</button></div>{newCategory ? <input autoFocus value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} placeholder="輸入新分類名稱" /> : <div className="category-options">{categories.map((category) => <button className={form.category === category ? "selected" : ""} onClick={() => setForm({ ...form, category })} key={category}>{category}</button>)}</div>}<small>{newCategory ? "輸入未使用過嘅分類名稱。" : "所有現有分類已展開，直接點選一個。"}</small></div><div className="quantity-unit-row"><label>正確數量<input inputMode="numeric" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value.replace(/\D/g, "") })} /></label><label>輸入單位<select value={form.unitMode} onChange={(event) => setForm({ ...form, unitMode: event.target.value as UnitMode })}><option value="package">箱／包</option><option value="base">{item?.unit ?? "件"}</option></select></label></div><div className="conversion-note">更正後入貨：<strong>{Number.isFinite(units) ? units : 0}</strong> {item?.unit}</div><div className="status-note">產品名稱、分類及入貨更正會一次過同步；原資料及更正人仍會保留。</div>{error && <p className="form-message error">{error}</p>}<button className="primary-button" onClick={save} disabled={busy || !form.quantity}>{busy ? "更正中…" : "確認更正"}</button></div></section></div>;
}
